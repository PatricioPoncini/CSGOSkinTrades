...
const { title, description, imgUrl, rarity, rarityName } = defineProps<{
  title: string,
  description: string,
  imgUrl: string,
  rarity: string,
...
const bgColor = computed(() => `bg-[${getRarityColor(rarity)}] px-2 py-1 text-primary-foreground rounded-full text-[14px] font-medium`);

</script>

<template>
  <div className="w-full bg-white max-w-sm rounded-lg overflow-hidden shadow-lg transition-all hover:shadow-xl">
